...
namespace LocalTranslateHub.Tests;

public sealed class ApiTests : IDisposable
{
    private readonly string _tempDirectory = Path.Combine(Path.GetTempPath(), "lth-tests-" + Guid.NewGuid());
    ...
    public ApiTests()
    {
        ...
        _previousCachePath = Environment.GetEnvironmentVariable("LTH_LocalTranslateHub__CachePath");
        _previousDefaultProvider = Environment.GetEnvironmentVariable("LTH_LocalTranslateHub__DefaultProvider");
        _previousDefaultSpeechProvider = Environment.GetEnvironmentVariable("LTH_LocalTranslateHub__Speech__DefaultProvider");
        _previousUrls = Environment.GetEnvironmentVariable("LTH_Urls");

        Environment.SetEnvironmentVariable(
            "LTH_LocalTranslateHub__CachePath",
            Path.Combine(_tempDirectory, "translations.sqlite"));
        Environment.SetEnvironmentVariable("LTH_LocalTranslateHub__DefaultProvider", "mock");
        Environment.SetEnvironmentVariable("LTH_LocalTranslateHub__Speech__DefaultProvider", "mock");
        Environment.SetEnvironmentVariable("LTH_Urls", "http://localhost:0");
        ...
        _factory = new WebApplicationFactory<Program>()
            .WithWebHostBuilder(builder =>
            {
                builder.ConfigureAppConfiguration((_, config) =>
                {
                    ...
                    {
                        ["LocalTranslateHub:DefaultProvider"] = "mock",
                        ["LocalTranslateHub:Speech:DefaultProvider"] = "mock",
                        ["LocalTranslateHub:CachePath"] = Path.Combine(_tempDirectory, "translations.sqlite")
                    });
                });
            });
    }
    ...
    public async Task Viewer_ReturnsMobileDisplayPage()
    {
        ...
        Assert.Contains("LocalTranslateHub Viewer", html);
    ...
    public async Task App_ReturnsWorkbenchPage()
    {
        ...
        Assert.Contains("LocalTranslateHub App", html);
    ...
    public void Dispose()
    {
        ...
        Environment.SetEnvironmentVariable("LTH_LocalTranslateHub__CachePath", _previousCachePath);
        Environment.SetEnvironmentVariable("LTH_LocalTranslateHub__DefaultProvider", _previousDefaultProvider);
        Environment.SetEnvironmentVariable("LTH_LocalTranslateHub__Speech__DefaultProvider", _previousDefaultSpeechProvider);
        Environment.SetEnvironmentVariable("LTH_Urls", _previousUrls);
