...
{
    private readonly string _tempDirectory = Path.Combine(Path.GetTempPath(), "yomibridge-tests-" + Guid.NewGuid());
    ...
    public ApiTests()
    {
        ...
        _previousCachePath = Environment.GetEnvironmentVariable("YB_YomiBridge__CachePath");
        _previousDefaultProvider = Environment.GetEnvironmentVariable("YB_YomiBridge__DefaultProvider");
        _previousDefaultSpeechProvider = Environment.GetEnvironmentVariable("YB_YomiBridge__Speech__DefaultProvider");
        _previousUrls = Environment.GetEnvironmentVariable("YB_Urls");

        Environment.SetEnvironmentVariable(
            "YB_YomiBridge__CachePath",
            Path.Combine(_tempDirectory, "translations.sqlite"));
        Environment.SetEnvironmentVariable("YB_YomiBridge__DefaultProvider", "mock");
        Environment.SetEnvironmentVariable("YB_YomiBridge__Speech__DefaultProvider", "mock");
        Environment.SetEnvironmentVariable("YB_Urls", "http://localhost:0");
        ...
        _factory = new WebApplicationFactory<Program>()
            .WithWebHostBuilder(builder =>
            {
                builder.ConfigureAppConfiguration((_, config) =>
                {
                    ...
                    {
                        ["YomiBridge:DefaultProvider"] = "mock",
                        ["YomiBridge:Speech:DefaultProvider"] = "mock",
                        ["YomiBridge:CachePath"] = Path.Combine(_tempDirectory, "translations.sqlite")
                    });
                });
            });
    }
    ...
    public async Task Viewer_ReturnsMobileDisplayPage()
    {
        ...
        Assert.Contains("YomiBridge Viewer", html);
    ...
    public async Task App_ReturnsWorkbenchPage()
    {
        ...
        Assert.Contains("YomiBridge App", html);
    ...
    public void Dispose()
    {
        ...
        Environment.SetEnvironmentVariable("YB_YomiBridge__CachePath", _previousCachePath);
        Environment.SetEnvironmentVariable("YB_YomiBridge__DefaultProvider", _previousDefaultProvider);
        Environment.SetEnvironmentVariable("YB_YomiBridge__Speech__DefaultProvider", _previousDefaultSpeechProvider);
        Environment.SetEnvironmentVariable("YB_Urls", _previousUrls);
