...
LocalTranslateHub 的長期定位不是單純的「OCR 翻譯 API」，而是「即時螢幕語境翻譯器」：在不打斷玩家沉浸感的前提下，記住術語、角色、劇情脈絡與使用者修正，讓翻譯越用越一致、越用越貼近作品語氣。

這份文件描述產品方向與分期路線。它不是目前功能清單；凡是標在未來階段的能力，都代表尚未實作。

## 目前狀態

目前 LocalTranslateHub 已經是一個可用的本地翻譯 gateway，重點放在 MORT Custom API 相容與本地模型翻譯。
...
LocalTranslateHub 的護城河應該是應用層記憶，而不是單一模型：
...
第一階段完成後，LocalTranslateHub 應該是可靠的「本地翻譯 gateway」，可以接 MORT、OCR 工具或其他上游文字來源。
...
第二階段完成後，LocalTranslateHub 應該能提供「一致、有記憶、可修正」的語境翻譯。
...
第三階段完成後，LocalTranslateHub 才會接近完整的「即時螢幕語境翻譯器」。
...
```text
LocalTranslateHub 是一個會記得術語、劇情與修正的即時螢幕語境翻譯器。
```
