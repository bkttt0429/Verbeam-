...
{
    private readonly YomiBridgeOptions _options;
        ...
        CancellationToken cancellationToken)
    {
        ...
        await foreach (var chunk in DownloadYouTubeAudioChunksAsync(sourceUrl, chunkSeconds, cancellationToken))
        {
            ...
            {
                throw new ArgumentException($"audio chunk is too large. Max size is {_options.Speech.MaxAudioBytes} bytes. Lower YomiBridge:Speech:YouTube:AudioChunkSeconds.");
            }
    ...
    public async Task<IReadOnlyList<SpeechSegment>> TryLoadYouTubeCaptionsAsync(
        ...
        CancellationToken cancellationToken)
    {
        var tempDirectory = Path.Combine(Path.GetTempPath(), $"yomibridge-captions-{Guid.NewGuid():N}");
    ...
    {
        var tempDirectory = Path.Combine(Path.GetTempPath(), $"yomibridge-youtube-audio-{Guid.NewGuid():N}");
        Directory.CreateDirectory(tempDirectory);
