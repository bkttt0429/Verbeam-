...
public sealed class ExternalCommandOcrProvider : IOcrProvider
{
        ...
        CancellationToken cancellationToken)
    {
        ...
        {
            throw new InvalidOperationException("External OCR provider is not configured. Set YomiBridge:Ocr:External:FileName.");
        }

        var extension = ExtensionFromMimeType(request.ImageMimeType);
        var imagePath = Path.Combine(Path.GetTempPath(), $"yomibridge-ocr-{Guid.NewGuid():N}{extension}");
