...
public sealed class ExternalCommandSpeechProvider : ISpeechProvider
{
        ...
        CancellationToken cancellationToken)
    {
        ...
        {
            throw new InvalidOperationException("External ASR provider is not configured. Set YomiBridge:Speech:External:FileName.");
        }

        var extension = ExtensionFromMimeType(request.AudioMimeType);
        var audioPath = Path.Combine(Path.GetTempPath(), $"yomibridge-asr-{Guid.NewGuid():N}{extension}");
