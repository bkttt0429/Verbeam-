...
builder.Configuration.AddEnvironmentVariables(prefix: "LTH_");
builder.WebHost.UseUrls(builder.Configuration["Urls"] ?? "http://localhost:5757");

var options = builder.Configuration.GetSection("LocalTranslateHub").Get<LocalTranslateHubOptions>()
    ?? new LocalTranslateHubOptions();
...
static IReadOnlyList<OcrEngineDescriptor> BuildPlannedOcrSet(LocalTranslateHubOptions options)
{
