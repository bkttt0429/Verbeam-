...
builder.Services.AddSingleton(new TranslationConfigurationCatalog(options));
    ...
    CancellationToken cancellationToken) =>
{
    try
    {
        ...
        {
            var models = await ollamaModels.ListAsync(descriptor.Name, cancellationToken);
            return Results.Ok(catalog.EnrichModels(descriptor.Name, models));
        }

        var model = Pick(descriptor.DefaultModel, descriptor.Name);
        return Results.Ok(catalog.EnrichModels(descriptor.Name, new[]
        {
