...
<body>
  <div class="shell">
    <header class="topbar">
      <div class="brand"><span class="mark">OC</span><span>Verbeam</span></div>
            ...
            <div class="settings-tab-body">
              <section class="settings-section">
                ...
                <div data-component="settings-v2-list">
                  <div data-component="settings-v2-row">
                    ...
                    <div data-slot="settings-v2-row-control">
                      <div class="settings-control-grid">
                        ...
                        <div class="field"><label for="target">target</label><select id="target"></select></div>
                      </div>
                  ...
                  <div data-component="settings-v2-row">
                    <div data-slot="settings-v2-row-copy">
                      <div data-slot="settings-v2-row-title">Prompt summary</div>
                      <div data-slot="settings-v2-row-description">Active preset metadata used for the model prompt.</div>
                    </div>
                    <div data-slot="settings-v2-row-control">
                      <div class="settings-status-stack">
                        <div class="status-line"><span>preset</span><strong id="promptPresetName">-</strong></div>
                        <div class="status-line"><span>version</span><strong id="promptPresetVersion">-</strong></div>
                        <div class="status-line"><span>focus</span><strong id="promptPresetDescription">-</strong></div>
                      </div>
                    </div>
                  </div>
                  <div data-component="settings-v2-row">
              ...
              <section class="settings-section">
                ...
                <div data-component="settings-v2-list">
                  ...
                  <div data-component="settings-v2-row">
                    ...
                    <div data-slot="settings-v2-row-control">
                      <div class="settings-control field"><label for="ocrLanguage">language</label><input id="ocrLanguage" list="translationLanguageList" value="ja"></div>
                    </div>
                  </div>
                  <div data-component="settings-v2-row">
                    <div data-slot="settings-v2-row-copy">
  ...
  <script>
    ...
    function populateLanguageSelect(select, languages, fallback) {
      select.innerHTML = "";
      for (const item of languages || []) {
        const option = document.createElement("option");
        option.value = item.code;
        option.textContent = `${item.code} · ${item.nativeName || item.displayName}`;
        option.title = item.promptName || item.displayName || item.code;
        select.appendChild(option);
      }
    ...
    function populateLanguageDatalist(languages) {
      ...
      for (const item of languages || []) {
        const option = document.createElement("option");
        option.value = item.code;
        option.label = item.nativeName ? `${item.displayName} / ${item.nativeName}` : item.displayName;
    ...
    function chooseRecommendedModel() {
      ...
      return models.find(item => item.isRecommended && item.isInstalled)
        || models.find(item => item.isRecommended && item.isDefault)
        || models.find(item => item.isRecommended)
    ...
    function syncModelRecommendation() {
      ...
      $("applyRecommendedModelButton").disabled = !recommended || input.value.trim().toLowerCase() === recommended.name.toLowerCase();
    }
    ...
    function populateModelList(models, fallback) {
      ...
      const defaultModel = (models || []).find(item => item.isDefault)?.name || recommendedModel?.name || fallback || names[0] || "";
    ...
    async function boot() {
      try {
        const [health, providers, ocrEngines, speechEngines, presets, glossaries, languages, latest] = await Promise.all([
          api("/health"),
          api("/providers"),
          api("/ocr/engines"),
          api("/asr/engines"),
        ...
        populateLanguageSelect($("source"), state.translationLanguages, health.defaultSource || "ja");
    ...
    $("translationModel").addEventListener("input", () => {
      $("metricModel").textContent = $("translationModel").value || "-";
      syncModelRecommendation();
    });
