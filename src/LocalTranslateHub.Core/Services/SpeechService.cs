...
public sealed class SpeechService
{
        ...
        CancellationToken cancellationToken)
    {
        ...
        await foreach (var chunk in DownloadYouTubeAudioChunksAsync(sourceUrl, chunkSeconds, cancellationToken))
        {
            ...
            {
                throw new ArgumentException($"audio chunk is too large. Max size is {_options.Speech.MaxAudioBytes} bytes. Lower LocalTranslateHub:Speech:YouTube:AudioChunkSeconds.");
            }
    ...
    public async Task<IReadOnlyList<SpeechSegment>> TryLoadYouTubeCaptionsAsync(
        ...
        CancellationToken cancellationToken)
    {
        var tempDirectory = Path.Combine(Path.GetTempPath(), $"lth-captions-{Guid.NewGuid():N}");
    ...
    {
        var tempDirectory = Path.Combine(Path.GetTempPath(), $"lth-youtube-audio-{Guid.NewGuid():N}");
        Directory.CreateDirectory(tempDirectory);
