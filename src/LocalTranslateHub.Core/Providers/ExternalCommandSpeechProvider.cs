...
public sealed class ExternalCommandSpeechProvider : ISpeechProvider
{
        ...
        CancellationToken cancellationToken)
    {
        ...
        {
            throw new InvalidOperationException("External ASR provider is not configured. Set LocalTranslateHub:Speech:External:FileName.");
        }

        var extension = ExtensionFromMimeType(request.AudioMimeType);
        var audioPath = Path.Combine(Path.GetTempPath(), $"lth-asr-{Guid.NewGuid():N}{extension}");
