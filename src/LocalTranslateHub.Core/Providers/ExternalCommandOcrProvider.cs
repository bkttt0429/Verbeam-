...
public sealed class ExternalCommandOcrProvider : IOcrProvider
{
        ...
        CancellationToken cancellationToken)
    {
        ...
        {
            throw new InvalidOperationException("External OCR provider is not configured. Set LocalTranslateHub:Ocr:External:FileName.");
        }

        var extension = ExtensionFromMimeType(request.ImageMimeType);
        var imagePath = Path.Combine(Path.GetTempPath(), $"lth-ocr-{Guid.NewGuid():N}{extension}");
