...
$ApiProject = Join-Path $Root "src\Verbeam.Api\Verbeam.Api.csproj"
...
if ($env:VB_SKIP_ASR -eq "1") {
    Write-Host "Skipping FunASR startup because VB_SKIP_ASR=1"
}
...
if (Test-PortListening 5757) {
    Write-Host "Verbeam API is already listening on http://localhost:5757"
}
else {
    Start-Process `
        -FilePath "dotnet" `
    ...
    Write-Host "Started Verbeam API on http://localhost:5757"
}
